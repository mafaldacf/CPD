...
// 1st optimization
int cp_index = 0;
position *changed_positions = NULL;
int df_index = 0;
position *dead_foxes = NULL;
...
void insertNode(position *head, int i, int j, int index) {
    head[index].i = i;
    head[index].j = j;
}
...
void move_rabbit(int i, int j, int gen) {
    ...
    switch(world[p->i][p->j].entity) {
        case EMPTY:
            ...
            insertNode(changed_positions, p->i, p->j, cp_index++);
            break;
        case RABBIT:
...
void move_fox(int i, int j, int gen) {
    ...
    if(rabbit_cnt > 0) {
        ...
        if(world[i][j].starving_age >= fstarv)
            insertNode(dead_foxes, i, j, df_index++);
        return;
    }

    if(world[i][j].starving_age >= fstarv)
        insertNode(dead_foxes, p->i, p->j, df_index++);
...
void run_simulation() {
    ...
    for(gen = 1; gen <= ngenerations; gen++){
        ...
        for(i = 0; i < cp_index; i++) {
            p = &changed_positions[i];
            world[p->i][p->j].current_entity = world[p->i][p->j].entity;
        }
        cp_index = 0;
        ...
        for(i = 0; i < df_index; i++) {
            p = &dead_foxes[i];
            if(world[p->i][p->j].starving_age >= fstarv) {
                reset_position(p->i, p->j);
                nfox--;
            }
        }
        df_index = 0;
...
int main(int argc, char *argv[]) {
    ...
    max_animals = (M * N) - nrock;
    changed_positions = (position *) malloc(max_animals * sizeof(position));
    dead_foxes = (position *) malloc(max_animals * sizeof(position));
